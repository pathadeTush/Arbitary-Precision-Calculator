...
int fgreator(NUM l1, NUM l2){
    ...
    while(p->data != -2){p = p->next;}
    while(q->data != -2){q = q->next;}
...
void eq_num(NUM *l1, NUM *l2){
    ...
    if(search(*l1, -2) || search(*l2, -2)){
        ...
        if(!search(*l1, -2))  append(l1, -2);
        else if (!search(*l2, -2))  append(l2, -2);
...
NUM Mult(NUM *l1, NUM *l2){
    ...
    if(search(*l1, -2)){
        if(gtr == -1){
            while(last1->data != -2) {last1 = last1->prev;}
            f = last1->next;
        }
        else{
            while(last2->data != -2) {last2 = last2->prev;}
            f = last2->next;
        }
        Remove(l1, -2);
        Remove(l2, -2);
...
NUM Div(NUM *l1, NUM *l2){
    ...
    gtr = greator(*l1, *l2);
    while(precision != 15){               // precision = no. of digits after decimal points if number is not totally divisible.
        while(gtr != -2){
...
NUM Modulo(NUM *l1, NUM *l2){
    ...
    if(search(*l1, -2)){
        while(last->data != -2){last = last->prev;}
        f = last->next;
        Remove(l1, -2);
        Remove(l2, -2);
    }
